        ...
        {step === 1 ? (
          <form onSubmit={handleAuthSubmit} className="space-y-4">
            ...
            <div>
              ...
              <input name="email" required type="email" className="w-full px-4 py-2 border border-slate-200 rounded-xl focus:ring-2 focus:ring-blue-500 focus:outline-none" placeholder="patient@example.com" />
            </div>
            <div>
            ...
            <div className="text-center mt-4">
              <button type="button" onClick={() => { setMode(mode === 'login' ? 'register' : 'login'); setError(''); }} className="text-sm text-blue-600 hover:underline font-medium">
          ...
          <form onSubmit={handleOTP} className="space-y-4">
            ...
            <button type="button" onClick={() => setStep(1)} className="w-full text-slate-500 text-sm hover:text-slate-700 mt-2 font-medium">Go Back</button>
          </form>
        )}
      </div>
    </div>
...
export default function HealthSystemApp() {
  ...
  const [messages, setMessages] = useState([
    { id: 1, text: `Hello! I'm **Chatbot Eugene**, the assistant for the Health Monitoring System.\n\nI can help you understand our System. If you register or log in, I can also analyze your personal vitals and health history!\n\nHow can I assist you today?`, isBot: true }
  ]);
  const messagesEndRef = useRef(null);

  useEffect(() => { messagesEndRef.current?.scrollIntoView({ behavior: "smooth" }); }, [messages, isLoading, isChatOpen]);
            ...
            if (part.startsWith('[') && part.endsWith(']')) {
              ...
              return (
                <button 
                  key={j}
                  onClick={() => {
                    if (linkText.toLowerCase() === 'register' || linkText.toLowerCase() === 'login') {
                      setCurrentPage('auth');
                    } else if (currentUser) {
  ...
  const handleSendMessage = (e) => { e.preventDefault(); sendMessageToBot(inputText); };
    ...
    <div className="pt-16 pb-20 px-4 max-w-5xl mx-auto min-h-[70vh]">
      <div className="flex justify-between items-center mb-8">
        ...
        <div className="flex gap-3">
          <button onClick={() => setCurrentPage('medical-history')} className="flex items-center gap-2 bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 px-4 py-2 rounded-lg font-medium transition-colors shadow-sm">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16 items-center">
            ...
            <div className="hidden md:flex items-center space-x-8 text-sm font-medium text-slate-600">
              ...
              <button onClick={() => { setCurrentPage('home'); setTimeout(() => document.getElementById('features')?.scrollIntoView({behavior: 'smooth'}), 100); }} className="hover:text-blue-600 transition-colors">Features</button>
              
              {!currentUser ? (
                <button onClick={() => setCurrentPage('auth')} className="hover:text-blue-600 text-blue-600 font-bold transition-colors">Login / Register</button>
              ) : (
        ...
        <>
          <header className="relative overflow-hidden bg-white">
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-20 pb-24 relative z-10">
              <div className="md:w-2/3">
                ...
                <h1 className="text-5xl md:text-6xl font-extrabold text-slate-900 leading-tight mb-6">The Future of <span className="text-blue-600">Automated</span> Health Monitoring.</h1>
                <p className="text-xl text-slate-600 mb-8 max-w-2xl leading-relaxed">An all-in-one smart kiosk designed to deliver highly accurate vital signs, instant physical printouts, and AI-driven medical guidance.</p>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <div className="bg-slate-50 rounded-2xl shadow-md border border-slate-200 overflow-hidden flex flex-col">
                <div className="w-full h-80 md:h-[450px]">
                  <iframe src="https://maps.google.com/maps?q=St.%20John%20Paul%20II%20College%20of%20Davao&t=&z=15&ie=UTF8&iwloc=&output=embed" width="100%" height="100%" style={{ border: 0 }} allowFullScreen="" loading="lazy" referrerPolicy="no-referrer-when-downgrade" title="St. John Paul II College of Davao Location"></iframe>
                </div>
                <div className="p-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-8 bg-white">
                  <div className="flex-1 w-full flex items-start gap-3">
                    ...
                    <div><p className="text-slate-800 font-bold mb-1">St. John Paul II College of Davao</p><p className="text-slate-600 text-sm">Ecoland Dr, Matina, Davao City</p></div>
                  </div>
          ...
          <footer className="bg-slate-900 text-slate-400 py-12 border-t border-slate-800">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center text-xs">
              ...
              <p className="mt-2 text-slate-500">System Engineers: <span className="text-blue-400">Archie Abona</span>, <span className="text-blue-400">Jarold Camino</span> & <span className="text-blue-400">Kiervy Lawas</span></p>
            </div>
          </footer>
        </>
      )}
        ...
        <div className="fixed bottom-6 right-6 w-[360px] sm:w-[420px] h-[550px] bg-white rounded-2xl shadow-2xl border border-slate-100 flex flex-col overflow-hidden z-50">
          <div className="bg-gradient-to-r from-blue-600 to-blue-700 p-4 flex justify-between items-center text-white shadow-md z-10">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-white/20 rounded-full flex items-center justify-center border border-white/30"><Bot className="w-6 h-6" /></div>
              <div>
            ...
            <button onClick={() => setIsChatOpen(false)} className="hover:bg-white/20 p-2 rounded-full"><X className="w-5 h-5" /></button>
          </div>
          ...
          <div className="flex-1 p-5 overflow-y-auto bg-slate-50 space-y-5">
            {messages.map((msg) => (
              <div key={msg.id} className={`flex ${msg.isBot ? 'justify-start' : 'justify-end'} items-end gap-2`}>
                {msg.isBot && <div className="w-8 h-8 rounded-full bg-blue-100 flex-shrink-0 flex items-center justify-center border shadow-sm"><Bot className="w-4 h-4 text-blue-600" /></div>}
            ...
            {isLoading && (
              <div className="flex justify-start items-end gap-2">
                 <div className="w-8 h-8 rounded-full bg-blue-100 flex-shrink-0 flex items-center justify-center border shadow-sm"><Bot className="w-4 h-4 text-blue-600" /></div>
                <div className="bg-white border p-4 rounded-2xl rounded-bl-sm flex items-center gap-1.5 h-[42px]"><div className="w-1.5 h-1.5 bg-blue-400 rounded-full animate-bounce"></div><div className="w-1.5 h-1.5 bg-blue-400 rounded-full animate-bounce delay-150"></div><div className="w-1.5 h-1.5 bg-blue-400 rounded-full animate-bounce delay-300"></div></div>
              </div>
            )}
          ...
          {messages.length <= 2 && !isLoading && (
            <div className="px-3 pb-2 flex flex-wrap gap-2 bg-white pt-2 border-t border-slate-100">
              {(currentUser ? ["Review my vitals", "What is my health condition?"] : ["How to register?", "How to log in?", "Where are you located?"]).map((q, idx) => (
                <button key={idx} onClick={() => sendMessageToBot(q)} className="text-xs bg-blue-50 text-blue-600 border border-blue-200 px-3 py-1.5 rounded-full hover:bg-blue-600 hover:text-white transition-colors">{q}</button>
              ))}
            </div>
          )}

          <form onSubmit={handleSendMessage} className="p-3 bg-white border-t flex gap-2">
            <input type="text" value={inputText} onChange={(e) => setInputText(e.target.value)} placeholder="Ask Eugene..." className="flex-1 bg-slate-100 border focus:bg-white focus:ring-2 focus:ring-blue-500 rounded-full px-4 py-2.5 text-sm outline-none" />
            <button type="submit" disabled={!inputText.trim() || isLoading} className="w-11 h-11 bg-blue-600 text-white rounded-full flex items-center justify-center hover:bg-blue-700 disabled:opacity-50"><Send className="w-4 h-4 ml-0.5" /></button>
          </form>
        </div>
      )}
    </div>
